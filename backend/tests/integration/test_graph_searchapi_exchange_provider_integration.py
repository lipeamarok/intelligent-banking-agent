"""Integration: graph uses SearchApiExchangeProvider via GraphDependencies injection."""
...
def test_graph_uses_real_searchapi_provider_with_fake_http_client():
        ...
        response=FakeResponse(
            payload={
                "answer_box": {
                    ...
                    "to": {"price": 5.25, "currency": "United States Dollar"},
                }
            }
        )
    )

    exchange_provider = SearchApiExchangeProvider(
        api_key="test-key",
    ...
    state1 = dict(
        graph.invoke(
            {
                "session_id": "exch-searchapi-01",
                "trace_id": "trace-exch-searchapi-01",
                "cpf_candidate": ANA.cpf,
                "birth_date_candidate": ANA.data_nascimento,
            },
            config=_cfg("exch-searchapi-01-thread"),
        )
    )
    ...
    graph.update_state(
        _cfg("exch-searchapi-01-thread"),
        {
            "last_user_input": "quero cotacao do dolar",
            "exchange_request": {
                "base_currency": "BRL",
                "target_currency": "USD",
            },
    ...
    state2 = dict(graph.invoke(None, config=_cfg("exch-searchapi-01-thread")))
    for _ in range(3):
    ...
    assert quote.get("provider") == "searchapi"
