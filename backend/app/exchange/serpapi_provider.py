"""SerpAPI-backed exchange provider adapter."""
...
class SerpApiExchangeProvider(ExchangeProvider):
    """Fetches exchange quotes from SerpAPI with strict parsing and validation."""
    ...
    def __init__(
        ...
        if not isinstance(api_key, str) or api_key.strip() == "":
            raise ValueError("SerpApiExchangeProvider requires a non-empty api_key")
    ...
    def get_quote(self, base_currency: str, target_currency: str) -> dict[str, Any]:
        ...
        try:
            converter = data["answer_box"]["currency_converter"]
