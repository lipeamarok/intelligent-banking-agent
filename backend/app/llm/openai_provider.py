"""OpenAI provider adapter using chat/completions-style HTTP payloads."""
...
class OpenAIProvider(LLMProvider):
    """HTTP provider adapter for OpenAI chat completions."""
    ...
    def _build_default_client(self) -> Any:
        try:
            ...
            raise LLMProviderError("HTTP client unavailable for OpenAIProvider") from exc
        return httpx.Client()
    ...
    def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            raise LLMProviderError("OpenAI provider returned malformed response") from exc

        if not isinstance(content, str) or content.strip() == "":
            raise LLMProviderError("OpenAI provider returned empty content")
